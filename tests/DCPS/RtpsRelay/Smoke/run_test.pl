...
$test->process("relay1", "$ENV{DDS_ROOT}/bin/RtpsRelay", "-ReportStatistics 1 -DCPSConfigFile relay1.ini -ApplicationDomain 42 -StunPort 3478 -VerticalAddress 4444 -HorizontalAddress 127.0.0.1:11444 -UserData relay1 -StatisticsInterval 3" . $relay_security_opts);
$test->process("relay2", "$ENV{DDS_ROOT}/bin/RtpsRelay", "-ReportStatistics 1 -DCPSConfigFile relay2.ini -ApplicationDomain 42 -StunPort 3479 -VerticalAddress 5444 -HorizontalAddress 127.0.0.1:11544 -UserData relay2 -StatisticsInterval 3" . $relay_security_opts);
